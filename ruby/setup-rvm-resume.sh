...
#setup bash prompt
curl -O "https://raw.github.com/xtremelabs/xlt-scripts-bash/master/ruby/colours_bashprofile_rvm.sh" 1>/dev/null 2>/dev/null
cat colours_bashprofile_rvm.sh >> $HOME/.bash_profile
rm -rf colours_bashprofile_rvm.sh
...
if [[ "$MAC_VERSION" == 10.8.* ]]
then
   curl -O "http://assets.xtremelabs.com/xlt-scripts-bash/MacVim%20macosx%20108%20snapshot%2065.app" 1>/dev/null 2>/dev/null
   mvim_image_location="MacVim macosx 108 snapshot 65.app"
elif [[ "$MAC_VERSION" == 10.7.* ]]
then
   curl -O "http://assets.xtremelabs.com/xlt-scripts-bash/MacVim%20macosx%20107%20snapshot%2064.app" 1>/dev/null 2>/dev/null
